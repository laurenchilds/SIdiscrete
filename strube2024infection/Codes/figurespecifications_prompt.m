...
if generatefigures==1
    %% (02) USER PROMPT: WHICH TYPE OF FIGURES
        ...
        % (D) Batch Initial Condition Hydra Effect Plot?
            %-----Prompt-----% 
            usercatch=0;
            while usercatch==0
                batchhydra=input('Would you like to create a Batch Initial-Condition Hyda Effect Plot? Y=1/N=0> ');
                if any([0,1]==batchhydra) 
                    usercatch=1;
                else
                    fprintf('Invalid entry. Please try again.\n')
                end
    ...
    %% (04) BATCH INITIAL CONDITION HYDRA PROMPTS
    ...
    fprintf('\n\n TRAJECTORY PLOTS:\n')
        ...
        for i=1:1:trajectorycount
            ...
            %(B) User Prompt: Choose Bifurcation Parameter
                ...
                usercatch=0;
                while usercatch==0
                    pchoice1=input(sprintf('What value would you like to use for %s?> ',bp1));
                    if any([increm1(1):increm1(2):increm1(3)]==pchoice1)
                        usercatch=1;
                    else
                        fprintf('Invalid entry. Please try again.\n')
                    end
                ...
                usercatch=0;
                while usercatch==0
                    pchoice2=input(sprintf('What value would you like to use for %s?> ',bp2));
                    if any([increm2(1):increm2(2):increm2(3)]==pchoice2)
                        usercatch=1;
                    else
                        fprintf('Invalid entry. Please try again.\n')
                    end
                ...
                if (0<bifpar1) && (bifpar1<7)
                    traj_param(bifpar1,i)=pchoice1;
                elseif bifpar1>6
                    traj_init(bifpar1,i)=pchoice1;
                end

                if (0<bifpar2) && (bifpar2<7)
                    traj_param(bifpar2,i)=pchoice2;
                elseif bifpar2>6
                    traj_init(bifpar2,i)=pchoice2;
                end
